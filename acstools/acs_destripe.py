...
# LOCAL
from .utils_calib import SM4_MJD, extract_dark, extract_flash, extract_flatfield
...
class StripeArray:
    ...
    def configure_arrays(self):
        ...
        if self.ampstring == "ABCD":
            ...
            self.dq = np.concatenate((self.dq, self.hdulist["dq", 2].data[::-1, :]), axis=1)
        self.ingest_dark()
        self.ingest_flash()
    ...
    def ingest_flatfield(self):
        ...
        self.invflat = extract_flatfield(self.hdulist[0].header, self.hdulist[1])
...
def clean(
    ...
    if isinstance(mask1, str):
        ...
        for m in mask1:
            if isinstance(m, np.ndarray):
                ...
                raise TypeError("'mask1' must be a list of str or numpy.ndarray values.")
    else:
        raise TypeError("'mask1' must be either a str, or a numpy.ndarray, or a list of the two type of values.")
    ...
    if isinstance(mask2, str):
        ...
        for m in mask2:
            if isinstance(m, np.ndarray):
                ...
                raise TypeError("'mask2' must be a list of str or numpy.ndarray values.")
    else:
        raise TypeError("'mask2' must be either a str or a numpy.ndarray, or a list of the two type of values.")
    ...
    for image, maskfile1, maskfile2 in zip(flist, mlist1, mlist2):
        ...
        if fits.getval(image, "CCDAMP") == "ABCD" and (
            (mask1 is not None and mask2 is None) or (mask1 is None and mask2 is not None)
        ):
            raise ValueError("Both 'mask1' and 'mask2' must be specified or not specified together.")
...
def perform_correction(
    ...
    if Success:
        ...
        if STDDEVCorr > 1.5 * 0.9:
            LOG.warning(
                "perform_correction - STDDEV of applied de-stripe "
            ...
            LOG.info(f"perform_correction - STDDEV of applied de-stripe corrections {STDDEVCorr:.3g}.")

        if verbose:
            LOG.info(f"perform_correction - Estimated background: {Bkgrnd:.5g}.")
            LOG.info(f"perform_correction - Maximum applied correction: {MaxCorr:.3g}.")
            LOG.info(f"perform_correction - Effective number of clipping iterations: {NMaxIter}.")
            LOG.info(f"perform_correction - Effective number of additional (repeated) cleanings: {Nrpt}.")
            LOG.info(f"perform_correction - Total number of corrected rows: {NUpdRows}.")
...
def _mergeUserMaskAndDQ(dq, mask, dqbits):
    ...
    dqmask = bitfield_to_boolean_mask(dq, dqbits, good_mask_value=1, dtype=np.uint8)
...
def clean_streak(
    ...
    if stat == "pmode1":
        ...
        def getcorr():
            return 2.5 * SMedian - 1.5 * SMean

    elif stat == "pmode2":
        # "Original Pearson"-ian estimate for mode:
        def getcorr():
            return 3.0 * SMedian - 2.0 * SMean
        ...
        def getcorr():
            imstat = ImageStats(image.science[i][BMask], "mode", lower=lower, upper=upper, nclip=0)
            if imstat.npix != NPix:
                raise ValueError(f"imstate.npix ({imstat.npix}) != NPix ({NPix})")
            return imstat.mode
        ...
        def getcorr():
            imstat = ImageStats(image.science[i][BMask], "midpt", lower=lower, upper=upper, nclip=0)
            if imstat.npix != NPix:
                raise ValueError(f"imstate.npix ({imstat.npix}) != NPix ({NPix})")
            return imstat.midpt
    ...
    for rpt in range(nmax_rpt):
        ...
        if verbose:
            if Nrpt <= 1:
                ...
                LOG.info(f"clean_streak - Performing repeated image bias de-stripe #{Nrpt - 1}:")
        ...
        for i in range(image.science.shape[0]):
            ...
            SMean, SSig, SMedian, NPix, NIter, BMask = djs_iterstat(
                image.science[i], MaxIter=maxiter, SigRej=sigrej, Min=lower, Max=upper, Mask=mask_arr, lineno=i + 1
            )
            ...
            if NPix > 0:
                ...
                sigcorr2[i] = corr_scale[i] ** 2 * np.sum((image.err[i][BMask]) ** 2) / NPix**2
                cnpix[i] = NPix
                tnpix += NPix
                tnpix2 += NPix * NPix
                tcorr += corr[i] * NPix
        ...
        if atol is not None:
            ...
            if (
                nonconvi.shape[0] == nonconvi0.shape[0]
                and nonconvi.shape[0] == nonconvi_int.shape[0]
                and np.all(corr0[nonconvi] * corr[nonconvi] < 0.0)
                and Nrpt > 1
            ):
                LOG.warning(
                    "clean_streak - Repeat bias stripe cleaning\n"
        ...
        if verbose:
            if Nrpt <= 1:
                ...
                LOG.info(f"clean_streak - Repeated (#{Nrpt - 1}) image bias de-stripe: Done.")

    if verbose and Nrpt > 1:
        LOG.info('clean_streak - =====  Repeated de-stripe "residual" estimates:  =====')
        LOG.info(f"clean_streak - STDDEV of the last applied de-stripe corrections {STDDEVCorr:.3g}")
        LOG.info(f"clean_streak - Maximum of the last applied correction: {current_max_corr:.3g}.")
    ...
    if warn_maxiter:
        LOG.warning(
            f"clean_streak - Maximum number of clipping iterations specified by the user ({maxiter}) has been reached."
        )
...
def djs_iterstat(InputArr, MaxIter=10, SigRej=3.0, Max=None, Min=None, Mask=None, lineno=None):
    """
    ...
    if NGood == 1:
        imrow = _write_row_number(lineno=lineno, offset=0, pad=1)
        LOG.warning(f"djs_iterstat - Only one data point; cannot compute stats{imrow}")
        return 0, 0, 0, 0, 0, None
    if np.unique(InputArr).size == 1:
        imrow = _write_row_number(lineno=lineno, offset=0, pad=1)
        LOG.warning(f"djs_iterstat - Only one value in data; cannot compute stats{imrow}")
        return 0, 0, 0, 0, 0, None
    ...
    if NGood < 2:
        imrow = _write_row_number(lineno=lineno, offset=0, pad=1)
        LOG.warning(f"djs_iterstat - No good data points; cannot compute stats{imrow}")
        return 0, 0, 0, 0, 0, None
    ...
    while (Iter < MaxIter) and (NLast != NGood) and (NGood >= 2):
        ...
        if npix >= 2:
            ...
            FSig = np.sqrt(np.sum((1.0 * InputArr - FMean) ** 2 * Mask) / (npix - 1))
...
def main():
    ...
    parser = argparse.ArgumentParser(prog=__taskname__, description="Remove horizontal stripes from ACS WFC post-SM4 data.")
    parser.add_argument("arg0", metavar="input", type=str, help="Input file")
    parser.add_argument("arg1", metavar="suffix", type=str, help="Output suffix")
    parser.add_argument("maxiter", nargs="?", type=int, default=15, help="Max #iterations")
    parser.add_argument("sigrej", nargs="?", type=float, default=2.0, help="Rejection sigma")
    ...
    parser.add_argument("--mask1", nargs=1, type=str, default=None, help="Mask image for [SCI,1]")
    parser.add_argument("--mask2", nargs=1, type=str, default=None, help="Mask image for [SCI,2]")
    ...
    parser.add_argument("-c", "--clobber", action="store_true", help="Clobber output")
