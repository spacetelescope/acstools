...
def fit_streak_profile(yarr, p0, fit_background=True, plot_streak=False, max_width=None, ax=None, bounds=None):
    """
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", AstropyUserWarning)
        g_init = models.Gaussian1D(amplitude=amp0, mean=mean0, stddev=stdev0, bounds=bounds)
    ...
    if np.sum(sel) == 0:
        ...
        noise = nanmedian(np.abs(yarr[sel] - nanmedian(yarr[sel]))) / 0.67449
...
def filter_sources(
    ...
    min_persistence=0.5,
    persistence_chunk=100,
    min_persistence_snr=3,
):
    """
    ...
    for ii, p in enumerate(streak_positions):
        ...
        g, snr, width, mean_flux = fit_streak_profile(
            medarr, (None, dy_streak, 5), ax=use_ax, max_width=max_width, plot_streak=plot_streak
        )
        snrs[ii] = snr
        widths[ii] = width
        ...
        if (snr > minsnr) & (width < max_width):
            ...
            if check_persistence:
                ...
                LOG.info(
                    "breaking into {} sections for persistence check\nSection size for persistence check: {}".format(
                        nchunk, dx
                    )
                )  # noqa

                persistence[ii] = streak_persistence(subregion, int(dx), g.mean.value, g.stddev.value, max_width=max_width)
...
def create_mask(image, trail_id, endpoints, widths):
    ...
    for t, e, w in zip(trail_id, endpoints, widths):
        ...
        ry = (ry1 + ry2) / 2.0  # take avg, although they should be ~the same
...
def streak_persistence(cutout, dx, streak_y0, streak_stdev, max_width=None, plot_streak=False):
    """
    ...
    for ii in range(nsteps):
        ...
        g, snr, width, mean_flux = fit_streak_profile(
            chunk, guess, ax=ax, max_width=max_width, plot_streak=plot_streak, bounds=bounds
        )
        if ax:
    ...
    LOG.info(
        "Number of sections analyzed: {}\nNumber of sections that passed: {}\npersistance score: {}".format(
            len(persist), np.sum(persist), pscore
        )
    )
...
def _rot(image, angle, return_length, func):
    """Worker function for radon multiprocessing."""
    ...
    *,
    ...
    if circle:
        ...
        if np.any(image[outside_reconstruction_circle]):
            warnings.warn("Radon transform: image must be zero outside the reconstruction circle", AstropyUserWarning)
        # Crop image to make it square
        slices = tuple(
            slice(int(np.ceil(excess / 2)), int(np.ceil(excess / 2) + shape_min)) if excess > 0 else slice(None)
    ...
    if processes <= 1:
        with warnings.catch_warnings():
            ...
            for i, angle in enumerate(angles):
                cur_arr, cur_length = statfunc(padded_image, angle, True)
...
def update_dq(filename, ext, mask, dqval=16384, verbose=True, expand_mask=False):
    ...
    with fits.open(filename, mode="update") as pf:
        ...
        mask_larger = np.any([mask.shape[i] > dqarr.shape[i] for i in range(mask.ndim)])
        ...
        if not same_size:
            ...
            if mask_larger:
                LOG.error(
                    "Mask is larger than the DQ array in one or more "
                ...
                LOG.error(
                    "Cannot proceed due to size mismatch.\n"
                ...
                if factor_x != factor_y:
                    ...
                    raise ValueError(
                        "Mask and DQ array axis aspect ratios are"
                        " too different\n"
                        "Mask dimensions (x,y) = {}\n"
                        "Mask aspect ratio (x/y) = {}\n"
                        "DQ array dimensions (y,x) = {}\n"
                        "DQ array aspect ratio (x/y) = {}".format(mask.shape, mask_aspect, dqarr.shape, dq_aspect)
                    )
                ...
                mask = block_replicate(mask, factor, conserve_sum=False)
        ...
        if npix_updated > 0:
            ...
            pf["PRIMARY"].header.add_history(f"{time.ctime()} {__taskname__} v{__version__}({__vdate__})")
            pf["PRIMARY"].header.add_history(f"  Updated {npix_updated} px in EXT {ext} with DQ={dqval}")

    if verbose:
        fname = f"{filename}[{ext}]"

        print(
            f"DQ flag value is {dqval}\n"
...
def create_mrt_line_kernel(width, sigma, outfile=None, shape=(1024, 2048), plot=False, theta=None, processes=1):
    """
    ...
    rt = radon(image, theta=theta, circle=False, median=True, fill_value=np.nan, processes=processes, return_length=False)
    ...
    if do_interp:
        ...
        f = interpolate.RegularGridInterpolator(
            (rho_arr, theta_arr), cutout.data, bounds_error=False, fill_value=0, method="cubic"
        )

        # unable to provide 2D grid of points for interpolation. Switching to
        # 1D, and then will switch back
        points = [[r, t] for r, t in zip(np.ravel(new_rho_grid), np.ravel(new_theta_grid))]
